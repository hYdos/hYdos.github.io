...
Init = async function () {
    ...
    console.log('Starting Ginger3D build ' + BUILD);

    //Define image variables
    let img = await loadImage("testModel.png");

    //Define variables
    let vsText = await loadFile("shader.vs.glsl");

    let fsText = await loadFile("shader.fs.glsl");

    let modelObj = await loadJsonFile("testModel.json");
...
Run = function (vertexShaderText, fragmentShaderText, SusanImage, SusanModel) {
    model = SusanModel;
