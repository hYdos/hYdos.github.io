...
VERSION = 2;//increment by 1 with every major change; will make old datasets incompatible
SAVESLOT = 'beta';//actual slot will be 'legacySave-'+SAVESLOT
UPDATELOG = [
	{ date: '3/13/2020', title: 'Beta patch 1', text: ['more features to make the game more fun'] },
];
...
makeSeed = function (len) {
	var str = '';
	for (var i = 0; i < len; i++) { str += choose(chars); }
	return str;
}
...
G.Launch = function () {
	...
	G.Load = function (doneLoading) {
		...
		if (str != 'null' && str != '') {
			...
			var len = spl.length;
			for (var i = len - 1; i >= 0; i--) { if (spl[i] != '') G.gainTrait(G.know[parseInt(spl[i])]); }
	...
	G.NewGame = function (doneLoading, mods) {
		...
		if (!doneLoading) {
			...
			G.LoadMods(mods || ['data.js', 'misc.js', 'weather.js'], G.NewGame, true);
			return 0;
		}
				...
				text: 'Getting started', tooltip: 'Read a few tips on how to make it past the stone age.', onclick: function () {
					G.dialogue.popup(function (div) {
						return '<div style="width:480px;min-height:320px;height:75%;">' +
							...
							'<div class="bulleted">this is an early beta, so you don\'t have to worry about meeting other civilizations just yet</div>' +
	...
	G.createDebugMenu = function () {
		var str = '' +
			...
			'<br>' +
			G.button({
				text: 'ALMIGHTY', tooltip: 'Unlock every tech, trait and policy.', onclick: function () {
					for (var i in G.tech) {
	...
	G.Cheat = function () {
		if (!G.getSetting('debug')) { G.setSetting('debug', 1); G.middleText('- Debug mode activated -'); G.Message({ type: 'important', text: 'Debug mode activated.' }); return 'Debug mode activated.'; }
	...
	G.button = function (obj) {
		...
		if (obj.onclick || obj.tooltip || obj.tooltipFunc) {
			G.pushCallback(function (id, obj) {
				return function () {
					if (l(id)) {
	...
	G.textWithTooltip = function (text, tooltip, classes) {
		...
		G.pushCallback(function (id, tooltip) {
			return function () {
	...
	G.clickableText = function (text, func, classes) {
		...
		G.pushCallback(function (id, func) {
			return function (div) {
	...
	G.tabs =
		[
			...
			{ name: 'Territory Map', id: 'land', update: 'land', showMap: true, desc: 'View the world map, inspect explored territory and see your natural resources.' },
	...
	UPDATE LOG
	=======================================================================================*/
	G.updateLogPreface = '<div class="par"><b>NeverEnding Legacy</b> is currently in early beta, may feature strange and exotic bugs, and may be updated at any time.</div>' +
	...
	*/
		...
		if (div) {
			...
			if (siblings) {
				for (var i in siblings) {
					if (('settingButton-' + siblings[i]) != div.id) { l('settingButton-' + siblings[i]).classList.remove('on'); }
				}
			}
		}
	}
		...
		str += '<div class="scrollBox underTitle">' +
			(G.sequence == 'main' ?
				(
					...
					G.button({
						text: 'View loaded mods', tooltip: 'Check which mods are currently active on this game.', onclick: function () {
							G.dialogue.popup(function (div) {
								var str = '';
					...
					'<br>' +
					G.button({
						text: 'Wipe save', tooltip: 'Clear your save completely, removing your current game<br>and any achievements and game data.<br>Cannot be undone!', style: 'box-shadow:0px 0px 2px 1px #f00;', onclick: function () {
							G.dialogue.popup(function (div) {
					...
					'</div>' +
					G.button({
						text: 'New game', tooltip: 'Abandon your current game and start a new one.', onclick: function () {
							G.dialogue.popup(function (div) {
	...
	G.addCallbacks = function () {
		...
		for (var i = 0; i < len; i++) { G.Callbacks[i](); }
		G.Callbacks = [];
	}
	...
	G.update['res'] = function () {
		...
		for (var i in G.resCategories) {
			...
			if (div) div.onclick = function (cat) {
				return function () {
					cat.open = !cat.open;
					G.update['res']();
				};
		...
		for (var i = 0; i < len; i++) {
			...
			G.addTooltip(me.l, function (me) {
				return function () {
					return G.getResTooltip(me);
				};
			}(me.res), { offY: -8 });
			if (me.l) me.l.onclick = function (me) {
				return function () {
					if (G.getSetting('debug')) {
					...
					return function (div) {
						...
						if (instance.mode == 3) {
							...
							str += '</div><div class="buttonBox">' +
								G.button({
									text: 'Complete', tooltipFunc: function (me) { return function () { return '<div style="max-width:240px;padding:16px 24px;">You need ' + G.getCostString(me.finalStepCost, true, false, 1) + '.</div>'; } }(me), onclick: function (me) {
										return function () {
											var amount = 1;
											var success = true;
							...
							str += '</div><div class="buttonBox">' +
								G.button({
									text: 'Ascend', style: 'box-shadow:0px 0px 10px 1px #39f;', tooltipFunc: function (me) { return function () { return '<div style="max-width:240px;padding:16px 24px;"><div class="par">Ascending will end this game and let you create a new one.</div><div class="par">You will unlock permanent legacy bonuses for completion of this wonder.</div><div class="par">You may choose to do this later; click this wonder again to ascend at any time.</div><div class="par">Only do this when you\'re certain you\'re done with this world!</div></div>'; } }(me), onclick: function (me) {
										return function () {
											//ascend
											G.dialogue.close();
											var middleText = '';
				...
				if (div == G.widget.parent) G.widget.close();
					...
					G.widget.popup({
						func: function (widget) {
							...
							for (var i = 0; i < len; i++) {
								var land = G.land[i];
								l('tileEditLand-' + I).onmouseup = function (land) {
									return function () {
		...
		for (var i = 0; i < len; i++) {
			...
			if (reverse && effect.noReverse) { }//skip if this effect is set not to reverse
				...
				else if (oneTime && effect.type=='lock unit')
				{
					...
					for (var ii in effect.what) { G.getRes(ii).amount += (reverse ? -1 : 1) * effect.what[ii]; }
				}
				else if (!reverse && effect.type == 'show res')//no reverse
				{
					for (var ii in effect.what) { G.getRes(effect.what[ii]).visible = true; }
				}
				else if (!reverse && effect.type == 'show context')//no reverse
				{
					for (var ii in effect.what) { G.contextVisibility[effect.what[ii]] = true; }
				}
				else if (!reverse && effect.type == 'set name')//no reverse
				{
					for (var ii in effect.what) { G.getUnit(effect.what[ii]).displayName = effect.value; }
				}
				else if (effect.type == 'enable mode') {
					for (var ii in effect.what) { G.getUnit(effect.what[ii]).modes[effect.mode].disabled = (reverse ? effect.value : !effect.value); }
				}
				else if (!reverse && effect.type == 'make part of')//no reverse
				{
					for (var ii in effect.what) { G.makePartOf(effect.what[ii], effect.parent); }
				}
	...
	*/
	...
	G.ModLoadingDialogue = function () {
		G.dialogue.popup(function (div) {
			...
			else { str += '<div style="background:url(img/loadingBar.png);height:8px;position:absolute;bottom:0px;left:0px;right:0px;animation:driftRight 20s linear infinite;"></div>'; }
			str += '</div>';
			return str;
		}, 'noClose noFade');
	}
	...
	G.AddData = function (obj) {
		...
		if (!mod['name'] && !mod['func']) { mod.error = 'This mod is lacking a name and a function.'; }
