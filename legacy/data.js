G.AddData({
	name: 'Amost Default Neverending Legacy',
	author: 'Hydos',
	desc: 'Default dataset',
	engineVersion: 2,
	manifest: 0,
	func: function () {
		/*
		...
		*/
		...
		G.funcs['new game'] = function () {
			var str = 'Your name is ' + G.getName('ruler') + '' + (G.getName('ruler').toLowerCase() == 'jesus' ? ' <i>(but that\'s not you, is it?)</i>' : '') + ', ruler of ' + G.getName('civ') + '. Your tribe is primitive, but full of hope.<br>The first year of your legacy has begun. May it stand the test of time.';
		...
		G.resCategories = {
			...
			'demog': {
				name: 'Demographics',
				base: ['baby', 'child', 'teen', 'adult', 'elder', 'worker', 'sick', 'wounded'],
			...
			tick: function (me, tick) {
				...
				if (me.amount > 0) {
					...
					if (tick % 3 == 0 && G.checkPolicy('disable eating') == 'off') {
						//drink water
						var toConsume = 0;
						var weights = { 'baby': 0.1, 'child': 0.3, 'teen': 0.4, 'adult': 0.5, 'elder': 0.5, 'sick': 0.4, 'wounded': 0.4 };
						...
						if (lacking > 0)//are we out of water?
						{
							...
							if (lacking > 0 && G.checkPolicy('disable aging') == 'off')//are we also out of muddy water?
							{
								...
								var died = 0;
								var weights = { 'baby': 0.1, 'child': 0.2, 'teen': 0.4, 'adult': 0.5, 'elder': 1, 'sick': 0.3, 'wounded': 0.3 };//the elderly are the first to starve off
						...
						var weights = { 'baby': 0.2, 'child': 0.5, 'teen': 1.2, 'adult': 1, 'elder': 1, 'sick': 0.75, 'wounded': 0.75 };
						for (var i in weights) { toConsume += G.getRes(i).amount * weights[i]; }
						...
						if (lacking > 0)//are we out of food?
						{
							...
							if (lacking > 0 && G.checkPolicy('disable aging') == 'off')//are we also out of spoiled food?
							{
								...
								if (G.year < 1) toDie /= 5;//less deaths in the first year
								var died = 0;
								var weights = { 'baby': 0.1, 'child': 0.2, 'teen': 0.4, 'adult': 0.5, 'elder': 1, 'sick': 0.6, 'wounded': 0.6 };//the elderly are the first to starve off
								var sum = 0; for (var i in weights) { sum += weights[i]; } for (var i in weights) { weights[i] /= sum; }//normalize
								for (var i in weights) { var ratio = (G.getRes(i).amount / me.amount); weights[i] = ratio + (1 - ratio) * weights[i]; }
					...
					if (G.checkPolicy('disable aging') == 'off') {
						...
						var n = randomFloor(G.getRes('teen').amount * 0.003); G.gain('adult', n, 'aging up'); G.lose('teen', n, 'aging up');
						var n = randomFloor(G.getRes('child').amount * 0.002); G.gain('teen', n, 'aging up'); G.lose('child', n, 'aging up');
						var n = randomFloor(G.getRes('baby').amount * 0.005); G.gain('child', n, 'aging up'); G.lose('baby', n, 'aging up');

						//births
						var teenCount = G.getRes('teen').amount;
		...
		new G.Res({
			name: 'child',
			desc: '[child,Children] grow from [baby,Babies] over time.//After a while, they will grow up into [teen,Teens].//Children drink and eat half as much as adults.//Children do not count as [worker,Workers], unless special measures are in place.',
			startWith: 2,
			visible: true,
			partOf: 'population',
			icon: [3, 3],
		...
		new G.Res({
			name: 'teen',
			desc: '[teen,Teens] grow from [child,Children] over time.//After a while, they will grow up into [adult,Adults].//Some [teen, Teens] count as [worker,Workers].//most of the time, half of your [teen,Teens] can work',
			startWith: 2,
			visible: true,
			partOf: 'population',
			icon: [9, 3],
		...
		new G.Res({
			name: 'adult',
			desc: '[adult,Adults] grow from [teen,Teens] over time.//They eventually age into [elder,Elders].//Generally, adults make up most of your [worker,workforce].',
			startWith: 5,
			visible: true,
			partOf: 'population',
			icon: [4, 3],
		...
		new G.Res({
			...
			tick: function (me, tick) {
				...
				me.amount = me.amount + G.getRes('teen').amount / 2;
		...
		new G.Tech({
			name: 'tribalism',
			desc: '@unlocks [gatherer] and [farmer]@provides 5 [authority]<>Taking its roots in wild animal packs, [tribalism] is the organization of individuals into simple social structures with little hierarchy.',
			icon: [0, 1],
			startWith: true,
			effects: [
